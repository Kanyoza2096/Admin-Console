...
import { 
  Cpu, Zap, Activity, MessageCircle, Send, Globe, Server, Database, Search, X, AlertTriangle, Network
} from 'lucide-react';
...
function DataFlowVisualizer() {
  ...
  useEffect(() => {
    ...
    socket.on('new_message', () => { spawnPacketOnEdge('connectors', 'socketio'); burstSparks('connectors', '#22c55e', 4); setTotalEvents(prev => prev + 1); });
    socket.on('post_published', () => { spawnPacketOnEdge('scheduler', 'connectors'); burstSparks('scheduler', '#3b82f6', 6); setWorkflowActive(true); setTimeout(() => setWorkflowActive(false), 3000); setTotalEvents(prev => prev + 1); });
    socket.on('api_payload', () => { spawnPacketOnEdge('pipeline', 'scheduler'); setTotalEvents(prev => prev + 1); });
    socket.on('stats', () => { spawnPacketOnEdge('supabase', 'socketio'); setTotalEvents(prev => prev + 1); });

    socket.on('scan_complete', (data: any) => {
      spawnPacketOnEdge('gemini', 'command');
    ...
    >
      <div className="bg-brand-surface border border-brand-border rounded-2xl p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Data Flow Visualizer</h1>
            <p className="text-sm text-brand-text-muted mt-1">Real-time data pipeline and event flow</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex flex-col items-end">
            <span className="text-xs text-brand-text-muted">Events/s</span>
            <span className="text-xl font-mono font-bold text-brand-accent">{eventsPerSec}</span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-brand-surface border border-brand-border rounded-2xl p-4">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs text-brand-text-muted uppercase tracking-wider">Total Events</span>
            <Activity className="w-4 h-4 text-brand-primary" />
          </div>
          <div className="text-3xl font-mono font-bold text-brand-primary">{totalEvents.toLocaleString()}</div>
        </div>
        <div className="bg-brand-surface border border-brand-border rounded-2xl p-4">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs text-brand-text-muted uppercase tracking-wider">API Calls</span>
            <Zap className="w-4 h-4 text-yellow-400" />
          </div>
          <div className="text-3xl font-mono font-bold text-yellow-400">{stats?.apiCalls || 0}</div>
        </div>
        <div className="bg-brand-surface border border-brand-border rounded-2xl p-4">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs text-brand-text-muted uppercase tracking-wider">Failures</span>
            <AlertTriangle className="w-4 h-4 text-red-400" />
          </div>
          <div className="text-3xl font-mono font-bold text-red-400">{failureEvents}</div>
        </div>
      </div>

      <div className="relative w-full flex-1 bg-brand-surface border border-brand-border rounded-2xl overflow-hidden">
        <div 
        ...
        <div className="absolute top-3 right-3 z-30">
          <div className="relative">
            ...
            <input
              ...
              className="w-40 bg-brand-elevated/90 backdrop-blur-sm border border-brand-border rounded-lg pl-7 pr-2 py-1.5 text-[10px] text-brand-text font-mono focus:outline-none focus:border-brand-primary/50"
            />
            {searchQuery && (
        ...
        <div className="absolute bottom-16 right-4 w-28 h-20 bg-brand-elevated/80 backdrop-blur-sm border border-brand-border/50 rounded-lg overflow-hidden z-20 opacity-60 hover:opacity-100 transition-opacity">
        ...
        <div className="absolute bottom-4 left-4 right-4 flex justify-between items-center bg-brand-elevated/95 backdrop-blur-sm border border-brand-border rounded-lg px-4 py-2 z-30">
          ...
          <div className="flex items-center gap-4">
            {lastFailure && <span className="text-[9px] text-red-400/80 font-mono max-w-[200px] truncate hidden md:block">Last: {lastFailure}</span>}
